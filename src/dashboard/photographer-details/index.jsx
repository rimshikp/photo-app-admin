...
export default function PhotoGrapherDetails() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const photographerRes = await api.get(`/photos/photographers/${id}`);
        ...
        <div className={styles.profileSection}>
          <div className={styles.profileHeader}>
            ...
            <div className={styles.profileInfo}>
              ...
              <p className={styles.bio}>{photographer.bio || 'No bio available'}</p>
              
              <div className={styles.stats}>
                <div className={styles.statItem}>
                  <FontAwesomeIcon icon={faMoneyBillWave} className={styles.statIcon} />
                  <span>Total Earnings: <strong>₹{photographer?.totalEarnings.toFixed(2)}</strong></span>
                </div>
                <div className={styles.statItem}>
                  <FontAwesomeIcon icon={faImage} className={styles.statIcon} />
                  <span>Images: <strong>{photographer?.totalImages}</strong></span>
                </div>
                <div className={styles.statItem}>
                  <FontAwesomeIcon icon={faStar} className={styles.statIcon} />
                  <span>Rating: <strong>{photographer?.averageRating}</strong></span>
                </div>
              </div>
            </div>
          </div>
